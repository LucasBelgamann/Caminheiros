<template>
  <div style="width: 90vw; margin: 20px auto 0 auto;">
    <span class="text-h6" style="font-size: 15px; line-height: 0; color: #807676;">Usuários Inativos</span>
    <q-separator :class="mode ? 'default-bg-dark' : 'default-bg-ligth'" style="width: 90vw; margin: 10px auto;" />
    <div v-if="data.users.length" style="position: relative; height: 20vh;" v-for="user in data.users" :key="user.id">
      <q-card flat class="inactive-user-card" :class="mode ? 'dark-theme' : 'ligth-theme'">
        <q-card-section class="text-white row items-center" :class="mode ? 'dark-theme' : 'ligth-theme'">
        ...
        <q-card-actions align="right" :class="mode ? 'default-card-color-dark' : 'default-card-color-ligth'">
          <q-btn flat icon="fab fa-whatsapp"></q-btn>
          <q-btn flat icon="delete" color="negative"></q-btn>
        </q-card-actions>
      </q-card>
    </div>
  </div>
  <div v-if-else style="display: flex; justify-content: center; height: 20vh; align-items: center;">
    <p style="color: #a5a5a5; width: 200px; font-size: 15px;" class="text-center">Não encontramos nenhum usuário que esteja inativo!</p>
  </div>
</template>
